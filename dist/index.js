...
function flattenIconOptions(iconOptions2) {
  return iconOptions2.sizes.map((size) => ({
    ...size,
    offset: iconOptions2.offset ?? 0,
    pixelArt: iconOptions2.pixelArt ?? false,
  ...
  manifest() {
    ...
    properties.icons = icons.map((iconOptions2) => {
      ...
      return {
        ...
        type: "image/png",
        purpose: iconOptions2.purpose ?? defaultPurpose
      };
    });
...
var plugin_default = (image, config = {}) => ({
  ...
  async buildStart() {
    ...
    await Promise.all(response.images.map(async (image2) => await fsPromise.writeFile(path.join(dest, image2.name), image2.contents)));
    await Promise.all(response.files.map(async (file) => await fsPromise.writeFile(path.join(dest, file.name), file.contents)));
    await Promise.all(
