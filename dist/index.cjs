...
function svgDensity(metadata, width, height) {
  ...
  const currentDensity = _nullishCoalesce(metadata.density, () => ( 72));
  return Math.min(
    Math.max(
      1,
      currentDensity,
  ...
  if (Buffer.isBuffer(src)) {
    try {
      return [
        {
          data: src,
          metadata: await _sharp2.default.call(void 0, src).metadata()
        }
      ];
    } catch (error) {
    ...
    const buffer = await _promises.readFile.call(void 0, src);
...
function flattenIconOptions(iconOptions2) {
  return iconOptions2.sizes.map((size) => ({
    ...size,
    offset: _nullishCoalesce(iconOptions2.offset, () => ( 0)),
    pixelArt: _nullishCoalesce(iconOptions2.pixelArt, () => ( false)),
    background: asString(iconOptions2.background),
    transparent: iconOptions2.transparent,
    rotate: iconOptions2.rotate
  }));
}
function relativeTo(base, path2) {
  if (!base) {
    return path2;
  }
  const directory = base.endsWith("/") ? base : `${base}/`;
  const url = new URL(path2, new URL(directory, "resolve://"));
...
async function resize(source, width, height, pixelArt) {
  if (source.metadata.format === "svg") {
    ...
    return await _sharp2.default.call(void 0, source.data, options).resize({
      width,
      height,
      fit: _sharp2.default.fit.contain,
      background: "#00000000"
    }).toBuffer();
  } else {
    return await _sharp2.default.call(void 0, source.data).ensureAlpha().resize({
      width,
      height,
      fit: _sharp2.default.fit.contain,
      background: "#00000000",
...
function createBlankImage(width, height, background) {
  ...
  let image = _sharp2.default.call(void 0, {
    create: {
      width,
      height,
...
async function createFavicon(sourceset, name, iconOptions2) {
  const properties = flattenIconOptions(iconOptions2);
  const ext = _path.extname.call(void 0, name);
...
function transparentIcon(width, height) {
  return {
    sizes: [{ width, height: _nullishCoalesce(height, () => ( width)) }],
    offset: 0,
    background: false,
    transparent: true,
    rotate: false
...
function uniformIconOptions(options, iconsChoice, platformConfig) {
  ...
  if (Array.isArray(iconsChoice)) {
    ...
    result = platformConfig.filter((iconOptions2) => iconOptions2.name in iconsChoices).map((iconOptions2) => ({
      ...iconOptions2,
      ...iconsChoices[iconOptions2.name]
    }));
  } else if (typeof iconsChoice === "object") {
    result = platformConfig.filter((iconOptions2) => !iconOptions2.optional).map((iconOptions2) => ({
      ...iconOptions2,
      ...iconsChoice
    }));
  } else {
    result = platformConfig.filter((iconOptions2) => !iconOptions2.optional);
  }
  return result.map((iconOptions2) => ({
    pixelArt: options.pixel_art,
    ...iconOptions2,
    background: iconOptions2.background === true ? options.background : asString(iconOptions2.background)
  }));
}
var Platform = class {
  constructor(options, iconOptions2) {
    this.options = options;
    this.iconOptions = iconOptions2;
  }
  ...
  relative(path2) {
    return relativeTo(this.options.path, path2);
  }
  cacheBusting(path2) {
    ...
    const url = new URL(path2, "https://cache.busting");
...
var AndroidPlatform = class extends Platform {
  ...
  async createHtml() {
    return [
      ...
      this.options.appName ? `<meta name="application-name" content="${(0, import_escape_html.default)(this.options.appName)}">` : `<meta name="application-name">`
    ];
  }
  async shortcutIcons() {
  ...
  manifestFileName() {
    return _nullishCoalesce(_optionalChain([this, 'access', _2 => _2.options, 'access', _3 => _3.files, 'optionalAccess', _4 => _4.android, 'optionalAccess', _5 => _5.manifestFileName]), () => ( "manifest.webmanifest"));
  }
  manifest() {
    const { options } = this;
    ...
    properties.icons = icons.map((iconOptions2) => {
      const { width, height } = iconOptions2.sizes[0];
      return {
        src: this.cacheBusting(relativeTo(basePath, iconOptions2.name)),
        sizes: `${width}x${height}`,
        type: "image/png",
        purpose: _nullishCoalesce(iconOptions2.purpose, () => ( defaultPurpose))
      };
    });
...
var AppleIconPlatform = class extends Platform {
  ...
  async createHtml() {
    ...
    return [
      ...
      name ? `<meta name="apple-mobile-web-app-title" content="${(0, import_escape_html.default)(name)}">` : `<meta name="apple-mobile-web-app-title">`
    ];
  }
};
var SCREEN_SIZES = [
...
var ICONS_OPTIONS$3 = iconOptions();
...
var WindowsPlatform = class extends Platform {
  ...
  manifestFileName() {
    return _nullishCoalesce(_optionalChain([this, 'access', _6 => _6.options, 'access', _7 => _7.files, 'optionalAccess', _8 => _8.windows, 'optionalAccess', _9 => _9.manifestFileName]), () => ( "browserconfig.xml"));
  }
  browserConfig() {
    ...
    const contents = new import_xml2js.default.Builder({
...
var YandexPlatform = class extends Platform {
  ...
  manifestFileName() {
    return _nullishCoalesce(_optionalChain([this, 'access', _10 => _10.options, 'access', _11 => _11.files, 'optionalAccess', _12 => _12.yandex, 'optionalAccess', _13 => _13.manifestFileName]), () => ( "yandex-browser-manifest.json"));
  }
  manifest() {
    const basePath = this.options.manifestRelativePaths ? null : this.options.path;
...
_handleHTML = /* @__PURE__ */ new WeakMap();
_convertContent = /* @__PURE__ */ new WeakSet();
convertContent_fn = function(contents) {
  return (_nullishCoalesce(__privateGet(this, _options).emitBuffers, () => ( true))) && !Buffer.isBuffer(contents) ? Buffer.from(contents) : contents;
};
...
var directoryExists = async (path2) => {
  try {
    const stat = await new Promise((resolve, reject) => {
      _fs2.default.stat(path2, (err, result) => {
...
var plugin_default = (image, config = {}) => ({
  name: "laravel-vite-favicons",
  async buildStart() {
    const src = _path2.default.join(process.cwd(), image);
    let faviconOutputPath = "public/favicons";
    if (config.plugin) {
      if (config.plugin.faviconOutputPath !== void 0) {
        faviconOutputPath = config.plugin.faviconOutputPath;
      }
    }
    const dest = _path2.default.join(process.cwd(), faviconOutputPath);
    const response = await favicons(src, config.favicon);
    await _promises2.default.mkdir(dest, { recursive: true });
    await Promise.all(response.images.map(async (image2) => await _promises2.default.writeFile(_path2.default.join(dest, image2.name), image2.contents)));
    await Promise.all(response.files.map(async (file) => await _promises2.default.writeFile(_path2.default.join(dest, file.name), file.contents)));
    await Promise.all(
      response.html.map(async (html) => {
        let htmlOutput;
        if (config.plugin && config.plugin.faviconOutputPath !== void 0) {
          htmlOutput = html.replaceAll(">", ">\n");
        } else {
          htmlOutput = html.replaceAll(">", ">\n");
        }
        if (config.plugin && config.plugin.templateOutputPath !== void 0) {
          if (!await directoryExists(_path2.default.join(process.cwd(), config.plugin.templateOutputPath))) {
            await _promises2.default.mkdir(_path2.default.join(process.cwd(), config.plugin.templateOutputPath), { recursive: true });
          }
          await _promises2.default.writeFile(_path2.default.join(process.cwd(), config.plugin.templateOutputPath, "favicons.blade.php"), "");
          await _promises2.default.appendFile(_path2.default.join(process.cwd(), config.plugin.templateOutputPath, "favicons.blade.php"), htmlOutput);
        } else {
          if (!await directoryExists(_path2.default.join(process.cwd(), "resources/views/includes"))) {
            await _promises2.default.mkdir(_path2.default.join(process.cwd(), "resources/views/includes"), { recursive: true });
          }
          await _promises2.default.writeFile(_path2.default.join(process.cwd(), "resources/views/includes/favicons.blade.php"), "");
          await _promises2.default.appendFile(_path2.default.join(process.cwd(), "resources/views/includes/favicons.blade.php"), htmlOutput);
        }
      })
    );
  }
...
exports.default = plugin_default; exports.directoryExists = directoryExists;
/**
